...
export default defineEventHandler(async event => {
  ...
  if (streak.win) {
    badges.push({ icon: "🔥", count: Math.floor(streak.currentStreak / 3),message:"A gagné plus de 3 games d'affilées" })
  } else {
    badges.push({ icon: "😰", count: Math.floor(streak.currentStreak / 2),message:"A perdu plus de 3 games d'affilées" })
  }
  //check for shower badge
  const gamesCount = await getGamesCountByAccountByDay(account.id, 1)
  if (gamesCount?._count.LpUpdate) {
    badges.push({ icon: "🚿", count: Math.floor(gamesCount?._count.LpUpdate / 8),message:"A joué plus de 8 games dans la journée"  })
  }
  return badges
})
