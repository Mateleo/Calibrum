...
exec(`git rev-list --tags --skip=1 --max-count=1`, (error, commitCurentVersion, stderr) => {
  if (error) {
  ...
  exec(`git describe --abbrev=0 --tags ${commitCurentVersion}`, (error, CurrentVersion, stderr) => {
    if (error) {
      console.error(`Error executing command: ${error.message}`)
      return
    }
