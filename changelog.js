...
const mergeHeadHash = null;
try {
  mergeHeadHash = execSync("git rev-parse MERGE_HEAD").toString().trim();
  console.log(`Merge commit hash: ${mergeHeadHash}`);
} catch (error) {
  console.error(`Error finding MERGE_HEAD: ${error.message}`);
}
exec(
  mergeHeadHash
    ? `git log ${mergeHeadHash} --oneline --pretty=format:'%H$$%an$$%s'`
    ...
    if (features.length > 0) {
      ...
      features.forEach((e) => {
        changelog += ` - ${e.content} ([${e.id.substring(
          0,
          7
        )}](https://github.com/Mateleo/Calibrum/commit/${e.id})) by @${e.author}\n`;
      });
    }
